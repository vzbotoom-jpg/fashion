...
@section('content')
@if(session('status'))
    <div class="bg-primary/5 border-l-4 border-primary p-4 rounded-lg mb-6">
        <p class="text-sm text-primary-dark font-medium">{{ session('status') }}</p>
    </div>
@endif
...
<form method="POST" action="{{ route('password.email') }}" class="space-y-6">
    ...
    <!-- Info -->
    <div class="text-sm text-gray-600 bg-gray-50 border border-gray-100 p-4 rounded-lg">
        <p class="flex items-start">
            <!-- Lightbulb Icon -->
            <svg class="w-5 h-5 text-info mr-2 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
