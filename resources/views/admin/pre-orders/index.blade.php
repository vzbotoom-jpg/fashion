...
@section('title', 'Manajemen Pre-Order - Admin Panel')
@section('page_title', 'Pre-Order')
@section('page_subtitle', 'Kelola semua pre-order')

@section('content')
<div class="flex items-center justify-between mb-6">
    <p class="text-sm text-gray-500">Total: {{ $preOrders->total() }} pre-order</p>
</div>

<!-- Filters -->
<div class="bg-white rounded-xl shadow-sm p-4 mb-6">
    <form action="{{ route('admin.pre-orders.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse($preOrders as $preOrder)
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-6 py-4 text-sm text-gray-600">{{ $preOrder->user->name }}</td>
                        <td class="px-6 py-4 text-sm text-gray-600">{{ $preOrder->product->name }}</td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-sm text-gray-500">{{ $preOrder->created_at->format('d M Y') }}</td>
                        <td class="px-6 py-4 text-right">
                            <a href="{{ route('admin.pre-orders.show', $preOrder->id) }}" class="text-primary hover:underline text-sm flex items-center justify-end gap-1">
                                Detail
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
